...
def _normalize_msys_path(raw: str) -> str:
    """On Windows, convert Git-Bash / MSYS paths (/c/Users/...) to native (C:/Users/...)."""
    if sys.platform == "win32":
        m = _MSYS_DRIVE_RE.match(raw)
        if m:
            return m.group(1).upper() + ":/" + raw[len(m.group(0)) :]
    return raw
...
def main(argv: list[str]) -> int:
    ...
    sessions_list_path = pathlib.Path(_normalize_msys_path(argv[2]))
    session_paths = [
        pathlib.Path(_normalize_msys_path(line))
