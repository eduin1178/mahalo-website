...
# Find a working Python 3 interpreter. On Windows, `python3` on PATH is often
# the Microsoft Store stub (prints an install prompt and exits non-zero), so we
# probe candidates and pick the first one that actually runs.
PY=""
for cand in python3 py python; do
  if command -v "$cand" >/dev/null 2>&1 && "$cand" -c "import sys; assert sys.version_info[0]==3" >/dev/null 2>&1; then
    PY="$cand"
    break
  fi
done
[[ -z "$PY" ]] && { echo "python3 not found (no working Python 3 interpreter on PATH)" >&2; exit 3; }
...
set +e
"$PY" "$SCRIPT_DIR/extract_image.py" "$OUT" "$new_sessions_file"
py_rc=$?
set -e
