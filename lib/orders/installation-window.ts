/**
 * The three fixed installation time windows offered at checkout. The order
 * persists only the START hour of the chosen window (`scheduledAt`'s UTC hour);
 * the two-hour interval is a pure function of that start hour and is rendered
 * only in the scheduling step (selector + live review). Everywhere else
 * (confirmation, order email, webhook, admin) renders the start hour.
 */
...
export const INSTALLATION_WINDOW_START_HOURS = [8, 10, 14] as const;
export type InstallationWindowStartHour =
  (typeof INSTALLATION_WINDOW_START_HOURS)[number];

/** Type guard: true only for an allowed window start hour (8, 10, 14). */
